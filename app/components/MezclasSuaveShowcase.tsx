...
type FlavorKey = "vainilla" | "chocolate" | "fresa" | "yogurt";

const FLAVORS: { key: FlavorKey; label: string; swatch: string }[] = [
  { key: "vainilla",   label: "Vainilla",      swatch: "#FFF3A0" },
  { key: "chocolate",  label: "Chocolate",     swatch: "#3D1A00" },
  { key: "fresa",      label: "Fresa",         swatch: "#FF6B8A" },
  { key: "yogurt",     label: "Yogurt Griego", swatch: "#F0EBE0" },
];

const IMG: Record<FlavorKey, string> = {
  vainilla:  "/images/soft-vainilla.png",
  chocolate: "/images/soft-choco.png",
  fresa:     "/images/soft-fresa.png",
  yogurt:    "/images/soft-galon.png",
};

export default function MezclasSuaveShowcase({ onFlavorChange }: { onFlavorChange?: (f: FlavorKey) => void }) {
  const [flavor, setFlavor] = useState<FlavorKey>("vainilla");
  ...
  return (
    <div style={{
      ...
      {/* Galones */}
      <div style={{ display: "flex", gap: "clamp(0.4rem, 1.2vw, 0.9rem)", alignItems: "flex-end", justifyContent: "center", width: "100%" }}>
        {FLAVORS.map((f) => {
          ...
          return (
            <button
              ...
              aria-label={`Ver Mezcla Suave ${f.label}`}
              style={{
                position: "relative", flex: "0 0 22%", aspectRatio: "2/3",
